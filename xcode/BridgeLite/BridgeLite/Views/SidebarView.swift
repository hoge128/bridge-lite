            ...
            if let entry = selectedEntry {
                ScrollView {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        DuplicateSectionView(entryID: entry.id)
...
private struct DuplicateSectionView: View {
    ...
    private var duplicateKey: Data? { store.duplicateKeyByID[entryID] }
    private var peers: [PhotoEntry] {
        guard let key = duplicateKey,
              let members = store.duplicateGroups[key] else { return [] }
        return members.filter { $0 != entryID }.compactMap { store.entries[$0] }
    }

    var body: some View {
        let peerList = peers
        if !peerList.isEmpty, let key = duplicateKey {
            SectionBox(String(
                format: String(localized: "duplicate.section.title", defaultValue: "Duplicate Copies (%lld)"),
                peerList.count
            )) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(String(
                        format: String(localized: "duplicate.section.caption", defaultValue: "This file has %lld other byte-identical copies"),
                        peerList.count
                    ))
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                    ...
                    ForEach(peerList, id: \.id) { peer in
                        DuplicatePeerRow(
                            peer: peer,
                            isRecommended: store.duplicateRecommendedID[key] == peer.id,
                            store: store
                        )
                    }

                    Divider()
                    Button(String(localized: "duplicate.action.trash_all_others", defaultValue: "Move all other copies to Trash")) {
                        store.trashAllOtherDuplicates(of: entryID)
                    }
                    .buttonStyle(.borderless)
                    .font(.caption)
                    .foregroundStyle(.red)
...
private struct DuplicatePeerRow: View {
    let peer: PhotoEntry
        ...
        HStack(spacing: 4) {
            if isRecommended {
                Image(systemName: "star.fill")
                    ...
                    .help(String(localized: "duplicate.recommended", defaultValue: "Recommended"))
            } else {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(peer.filename)
                    ...
                    .lineLimit(1)
                Text(peer.url.deletingLastPathComponent().path(percentEncoded: false))
                    .font(.caption2)
                    .foregroundStyle(.tertiary)
                    .lineLimit(1)
                    .truncationMode(.head)
            ...
            Spacer()
            Button {
                NSWorkspace.shared.activateFileViewerSelecting([peer.url])
            } label: {
                Image(systemName: "magnifyingglass")
            }
            .buttonStyle(.borderless)
            .help(String(localized: "duplicate.action.reveal", defaultValue: "Show in Finder"))

            Button {
                store.trashDuplicate(id: peer.id)
            } label: {
                Image(systemName: "trash")
            }
            .buttonStyle(.borderless)
            .foregroundStyle(.red)
            .help(String(localized: "duplicate.action.trash", defaultValue: "Move to Trash"))
        }
        .padding(.vertical, 1)
    }
}
