...
inline void init_xmp_fn() {
        ...
        try {
            SXMPMeta::Initialize();
            // kXMPFiles_ServerMode skips camera-RAW file-format handler registration,
            // which throws std::runtime_error on macOS 26 (Darwin 25.x).
            // We combine it with kXMPFiles_IgnoreLocalText (original flag).
            SXMPFiles::Initialize(kXMPFiles_ServerMode | kXMPFiles_IgnoreLocalText);
            ...
            // Catch std::runtime_error (RawCameraException) from macOS 26+
            fprintf(stderr, "Failed to initialize XMP Toolkit: unknown exception\n");
        }
    #endif
}
